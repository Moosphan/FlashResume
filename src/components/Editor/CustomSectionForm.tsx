...
export default function CustomSectionForm() {
  ...
  return (
    <section className="space-y-4">
      <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">自定义区块</h2>
      ...
      {customSections.map((sec) => (
        ...
        >
          ...
          {/* 区块标题 */}
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">区块标题</label>
            <input
              type="text"
              value={sec.title}
              onChange={(e) => handleChange(sec.id, 'title', e.target.value)}
              placeholder="请输入区块标题"
          ...
          <div>
            ...
            <textarea
              value={sec.content}
              onChange={(e) => handleChange(sec.id, 'content', e.target.value)}
              placeholder="请输入区块内容"
              rows={4}
