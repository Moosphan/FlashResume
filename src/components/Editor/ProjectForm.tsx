...
export default function ProjectForm() {
  ...
  const handleEndDateBlur = (proj: Project) => {
    ...
    setDateErrors((prev) => ({
      ...prev,
      [proj.id]: result.valid ? undefined : result.error,
    }));
  };

  const inputCls =
    'min-h-[44px] w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm outline-none transition-colors duration-150 focus:border-primary focus:ring-1 focus:ring-primary dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-500';

  return (
    <section className="space-y-4">
      <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">项目经验</h2>
      ...
      {projects.map((proj) => (
        ...
        >
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">担任角色</label>
            <input
              type="text"
              value={proj.role}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                onBlur={() => handleEndDateBlur(proj)}
                className={`${inputCls} ${dateErrors[proj.id] ? 'border-red-500 focus:border-red-500 focus:ring-1 focus:ring-red-500' : ''}`}
              />
              {dateErrors[proj.id] && (
                <p className="mt-1 text-sm text-red-500">{dateErrors[proj.id]}</p>
              )}
            </div>
          </div>

          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">项目描述</label>
            <textarea
              value={proj.description}
              onChange={(e) => handleChange(proj.id, 'description', e.target.value)}
              placeholder="请描述项目内容、技术栈和成果"
              rows={3}
